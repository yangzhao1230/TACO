...
from dna_optimizers.mbo_optimizaer import mbo_optimizer, get_fitness_info
...
def parse_args():
    ...
    parser.add_argument("--level", type=str, default="easy", help="Difficulty level.")
    ...
    parser.add_argument("--tfbs_lambda", type=float, default=10, help="Lambda value for TFBS.")
...
def set_seed(seed):
    ...
    torch.cuda.manual_seed(seed)  # PyTorch GPU (if you are using CUDA)
...
def main():
    ...
    optimizer_args.tfbs_reward_path = f"reward_dict/results_{optimizer_args.task}_{optimizer_args.level}.json"
    ...
    optimizer_args.wandb_run_name = (
        ...
        f"prefix_{optimizer_args.prefix_label}_"
        f"lambda_{optimizer_args.tfbs_lambda}_"
        f"seed_{optimizer_args.seed}"
    )
    print(f"Optimizer Arguments: {optimizer_args}")
    print(f"run_name: {optimizer_args.wandb_run_name}")
    

        
    optimizer = mbo_optimizer(optimizer_args)
