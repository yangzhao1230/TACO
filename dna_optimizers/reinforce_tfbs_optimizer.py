...
class reinforce_tfbs_optimizer(BaseOptimizer):
    ...
    def _init(self, cfg):
        ...
        train_data = scripts.utils.load_csv(cfg.oracle_train_data_path)
        # sample 250 random sequences
        train_data_sampled = train_data.sample(250, random_state=42)
        train_seqs = train_data_sampled["Sequence"].tolist()
        train_one_hot = str_to_one_hot(train_seqs)

        self.explainer = shap.DeepExplainer(self.oracle, train_one_hot.to(self.device))
        # self.motif_embedding = nn.Embedding(len(self.motifs), self.agent.model.config.hidden_size)
        # self.motif_predictor = nn.Linear(self.agent.model.config.hidden_size, 3) # promote, neutral, suppress

    # def sample_gumbel_softmax(self, logits, temperature=1.0):
    #     gumbel_noise = -torch.log(-torch.log(torch.rand_like(logits)))
    #     return torch.softmax((logits + gumbel_noise) / temperature, dim=-1)


    # def predict_tfbs_prob(self, dna_list):
    #     pass
        ...
        while eval_strings < cfg.max_strings:
            ...
            for idx, tfbs_info in enumerate(tfbs_struc):
                ...
                for tfbs in tfbs_list:
                    ...
                    if (region_importance < 0).all():
                        # rewards[idx, start+2:end+2] += region_importance
                        rewards[start+2:end+2, idx] += region_importance
                    
                    # If all importance values are positive, add to rewards
                    elif (region_importance > 0).all():
                        rewards[start+2:end+2, idx] += region_importance
